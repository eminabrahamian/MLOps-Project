...
def test_bmitransformer_and_drop():
    """
    BMITransformer should compute weight/(height_m^2) and drop originals if drop=True.
    """
    df = pd.DataFrame({
        "weight": [60, 80],
        "height": [160, 180]  # cm
...
def test_interactionfeatures_pairs():
    """
    InteractionFeatures should add k*(k-1)/2 new columns for k=3 numeric columns.
    """
    df = pd.DataFrame({
        "a": [1, 2],
        "b": [3, 4],
...
def test_datetimefeatures_and_cyclic():
    """
    DateTimeFeatures should extract day_of_week, month, hour, hour_sin, hour_cos.
    """
    df = pd.DataFrame({
...
def test_feature_transformers_registry():
    """
    Ensure FEATURE_TRANSFORMERS dict produces transformer instances when called with config.
    """
    cfg = {
